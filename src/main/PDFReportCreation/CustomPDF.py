...
class CustomPDF(FPDF):
    ...
    def add_cover_page(self):
        ...
        print("Cover page dict:", cover_page_dict)

        title = cover_page_dict.get("title", "Cover Page")
        subtitle = cover_page_dict.get("subtitle", "")
        ...
        if image_path and os.path.exists(image_path):
            ...
            self.cell(0, 20, title, ln=1, align='C')
            self.ln(20)

            # Subtitle
            self.set_font('Arial', 'I', 16)
            self.cell(0, 20, subtitle, ln=1, align='C')
            self.ln(20)
        ...
        if image_path and os.path.exists(image_path):
            ...
            self.ln(40 + self.h_pt)  # Move below the image with proper spacing

    def print_chapter(self, ch_num, ch_title, chapter_data):
        if ch_num == 1 and "cover_page" in self.asParametersDict:
            self.add_cover_page()
            self.add_page()  # Add a blank page after the cover page
